...
def _test_fsdp_tiled_mlp(
    ...
    """
    Test FSDP-wrapped TiledMLP vs non-FSDP TiledMLP.
    This ensures FSDP doesn't break the tiled implementation.
    """
    # Init process group
...
def _test_fsdp_tiled_vs_torch_mlp(
    ...
    # Torch standard MLP + FSDP (using regular SwiGLU as baseline)
    torch_model = LigerSwiGLUMLP(config=config).to(device).to(dtype)
...
def _test_fsdp_tiled_geglu_mlp(
    ...
    This ensures FSDP doesn't break the tiled GEGLU implementation.
    """
    # Init process group
    torch.distributed.init_process_group(
        backend="nccl",
    ...
    # Reference: same weights, no FSDP
    ref_model = LigerTiledGEGLUMLP(config=config, num_shards=num_shards).to(device).to(dtype)
    ref_model.gate_proj.weight.data = G.clone()
    ref_model.up_proj.weight.data = U.clone()
    ref_model.down_proj.weight.data = D.clone()
